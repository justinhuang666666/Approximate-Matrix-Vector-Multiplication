...
from .utils import clamp_ste, round_ste
...
def _integer_quantize(
    ...
    if isinstance(x, (Tensor, ndarray)):
        return clamp_ste(round_ste(x.mul(scale)), int_min, int_max).div(scale)
    elif isinstance(x, int):
        return x
    else:
        return clamp_ste(round_ste(x * scale), int_min, int_max) / scale
