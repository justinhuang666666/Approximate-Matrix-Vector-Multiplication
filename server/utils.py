...
def compute_bleu_score_rank(device, model, rank, tokenizer, source_texts, target_texts):
    """
    Computes BLEU score for translations generated by the model with a specified rank.

    Args:
        device: The device (CPU or GPU) to run the model on.
        model: The translation model.
        rank: The rank to use for the SVD-based forward method.
        tokenizer: Tokenizer for text processing.
        source_texts: List of source texts to translate.
        target_texts: List of reference translations.

    Returns:
        BLEU score as a percentage.
    """
    translations = []
    for text in source_texts:
        # Tokenize and encode the source text, move to device
        inputs = tokenizer(text, return_tensors="pt", padding=True, truncation=True).to(device)
        input_ids = inputs['input_ids']
        attention_mask = inputs['attention_mask']

        # Prepare decoder input (start with the special <pad> token, typically ID=1)
        ...
        with torch.no_grad():
            outputs = model(
                ...
                rank=rank  # Pass rank to the custom wrapper
            )

        # Decode the output token IDs into text
        translation = tokenizer.decode(outputs.logits.argmax(dim=-1).squeeze(), skip_special_tokens=True)
        translations.append(translation)
